(function () {
  ...
  async function addUploadedPdfs() {
    ...
    const uploadedItems = pdfs.map(pdf => {
      ...
      const numericPdfUrl = pdf.ojs_article_id && pdf.ojs_galley_id ? `${productionHost}/index.php/${journalSlug}/article/view/${pdf.ojs_article_id}/${pdf.ojs_galley_id}.html?uploadedId=${encodeURIComponent(pdf.id)}` : '';
